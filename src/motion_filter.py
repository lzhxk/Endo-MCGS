...
class MotionFilter:
    ...
    def __init__(self, net: torch.nn.Module, video: DepthVideo, thresh: float = 2.5, device: str = "cuda:0"):
        ...
        self.count = 0
        # Cache for flow/translation data used by FoV adaptation
        self._prev_delta_for_fov = None
        self._prev_pose_for_fov = None
    ...
    def track(
        ...
        ### only add new frame if there is enough motion ###
            ...
            if delta.norm(dim=-1)[valid.bool()].mean().item() > self.thresh:
                ...
                # FoV Adaptation: store flow magnitude for the new keyframe
                # delta is at scale factor resolution (imh//8 x imw//8)
                flow_norm = delta.norm(dim=-1)  # [1, 1, H/8, W/8]
                valid_mask_bool = valid.bool()
                flow_sum = flow_norm[valid_mask_bool].sum().item()
                flow_count = valid_mask_bool.sum().item()
                if flow_count > 0:
                    vid_idx = self.video.counter.value
                    self.video.flow_norm_sum[vid_idx] = flow_sum
                    self.video.flow_pixel_count[vid_idx] = flow_count
                    # Store raw flow vectors for FoV adaptation visualization.
                    # delta is at scale_factor resolution: [1, 1, H/scale, W/scale, 2]
                    if hasattr(self.video, "flow_2d"):
                        flow_vec = delta[0, 0]  # [H, W, 2]
                        valid_mask = valid_mask_bool[0, 0]  # [H, W]
                        flow_vec = torch.where(
                            valid_mask[..., None],
                            flow_vec,
                            torch.zeros_like(flow_vec),
                        )
                        self.video.flow_2d[vid_idx] = flow_vec.permute(2, 0, 1).to(self.video.flow_2d.dtype)
