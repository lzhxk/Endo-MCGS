...
class DepthVideo:
    ...
    def __init__(self, cfg):
        ...
        # FoV Adaptation: shared storage for flow/translation data used in rho computation
        # flow_norm_sum: accumulated sum of flow magnitudes (at scale_factor resolution)
        self.flow_norm_sum = torch.zeros(buffer, device=device, dtype=torch.float).share_memory_()
        # flow_pixel_count: number of valid pixels for averaging
        self.flow_pixel_count = torch.zeros(buffer, device=device, dtype=torch.float).share_memory_()
        # flow_2d: raw optical flow vectors (previous -> current) used for FoV visualization/debugging.
        # Shape: [buffer, 2, H/scale_factor, W/scale_factor]
        self.flow_2d = torch.zeros(
            buffer,
            2,
            ht // s,
            wd // s,
        ...
        # translation_norm: norm of relative camera translation for each frame pair
        self.translation_norm = torch.zeros(buffer, device=device, dtype=torch.float).share_memory_()
        # foV adaptation tracking flags
        self.fov_adapted = torch.zeros(1, device=device, dtype=torch.int).share_memory_()  # 0=not done, 1=done
        self.fov_adapt_count = torch.zeros(1, device=device, dtype=torch.int).share_memory_()  # frame count when adaptation was triggered
        # Flag to indicate that FoV adaptation triggered a SLAM retry from frame 0
        self.fov_triggered_retry = False
    ...
    def remove(self, index) -> None:
        ...
        self.flow_norm_sum[index] = 0.0
        self.flow_pixel_count[index] = 0.0
        self.flow_2d[index] = 0.0
        self.translation_norm[index] = 0.0
