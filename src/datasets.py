...
class BaseDataset(Dataset):
    ...
    def depthloader(self, index: int) -> np.ndarray:
        ...
        elif ".tiff" in depth_path:  # NOTE chen: TartanAir depth files
            depth_data = np.array(Image.open(depth_path), dtype=np.float32)
            depth_data /= self.png_depth_scale
        else:
...
class Replica(BaseDataset):
    def __init__(self, cfg: DictConfig, device: str = "cuda:0"):
        ...
        self.color_paths = sorted(glob.glob(os.path.join(self.input_folder, "color/*.png")))
        # Set number of images for loading poses
        self.n_img = len(self.color_paths)
        self.load_poses(os.path.join(self.input_folder, "pose.txt"))
        ...
        if cfg.mode == "prgbd":
            ...
            self.depth_paths = sorted(glob.glob(os.path.join(self.input_folder, "depth/*.tiff")))
