...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None, device: str = "cuda:0"):
        ...
        self.xyz_gradient_accum_abs = torch.empty(0, device=self.device)
        ...
        # Manifold projection controller (SDF-based Gaussian projection)
        self._manifold_ctrl = None

    def set_manifold_controller(self, ctrl):
        """Attach a ManifoldProjectionController for SDF-based Gaussian projection."""
        self._manifold_ctrl = ctrl
    ...
    @property
    def get_projected_xyz(self):
        """Return SDF-projected Gaussian centres if manifold projection is enabled."""
        if self._manifold_ctrl is not None:
            return self._manifold_ctrl.project(self._xyz)
        return self._xyz
    ...
    def get_rotation_matrix(self):
        ...
        from ...datasets import quaternion_to_matrix
        return quaternion_to_matrix(self.get_rotation)
    
    def get_smallest_axis(self, return_idx=False):
        """Get the smallest axis of the Gaussian ellipsoid (used for normal estimation)"""
    ...
    def get_normal(self, view_cam):
        """Get surface normal for each Gaussian (PGSR compatibility)"""
    ...
    def training_setup(self, training_args):
        ...
        # PGSR-style densification thresholds (with safe defaults for backward compatibility)
        # max screen-space radius threshold used for absolute-gradient splitting
        self.abs_split_radii2D_threshold = getattr(training_args, "abs_split_radii2D_threshold", 0.0)
        # limit for number of absolute-gradient based split points (optional)
        self.max_abs_split_points = getattr(training_args, "max_abs_split_points", 1_000_000)
        # global cap to avoid explosion of points (optional)
        self.max_all_points = getattr(training_args, "max_all_points", 10_000_000)
    ...
    def get_avg_scale(self, factor: float = 1.0, kfIdx: Optional[torch.Tensor | List[int]] = None) -> float:
        """Get average scene depth to derive a scale; optionally restrict to specific keyframes (DROID-Splat style)."""
        points = self.get_xyz
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        """DROID-Splat style clone: duplicate points with large gradients but small scale."""
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, scale_std=1.0):
        ...
        # Clone and Split using vanilla 3DGS strategy
    ...
    def add_densification_stats(
        ...
        """Accumulate gradient statistics; optionally weight by number of touched pixels (DROID-Splat style)."""
        ...
        # Stats only; no structural changes here

    # --- PGSR-compatible densification APIs ---
    def add_densification_stats_pgsr(self, viewspace_point_tensor: torch.Tensor, viewspace_point_tensor_abs: torch.Tensor, update_filter: torch.Tensor):
        """PGSR-style stats accumulation: keep both relative and absolute gradient accumulators."""
        # Only use xy components as in PGSR implementation
        if viewspace_point_tensor.grad is None or viewspace_point_tensor_abs.grad is None:
            return
        # Align lengths defensively
        num_pts = self.get_xyz.shape[0]
        m = min(num_pts, update_filter.shape[0], viewspace_point_tensor.grad.shape[0], viewspace_point_tensor_abs.grad.shape[0])
        if m == 0:
            return
        mask = update_filter[:m]
        grad_rel = viewspace_point_tensor.grad[:m, :2]
        grad_abs = viewspace_point_tensor_abs.grad[:m, :2]
        self.xyz_gradient_accum[:m][mask] += torch.norm(grad_rel[mask], dim=-1, keepdim=True)
        self.xyz_gradient_accum_abs[:m][mask] += torch.norm(grad_abs[mask], dim=-1, keepdim=True)
        self.denom[:m][mask] += 1
        self.denom_abs[:m][mask] += 1

    def densify_and_split_pgsr(self, grads: torch.Tensor, grad_threshold: float, grads_abs: torch.Tensor, grad_abs_threshold: float, scene_extent: float, max_radii2D: torch.Tensor, N: int = 2):
        n_init_points = self.get_xyz.shape[0]
        padded_grad = torch.zeros((n_init_points), device=self.device)
        padded_grad[: grads.shape[0]] = grads.squeeze()
        ...
        # If still room, additionally select by abs gradient with radii2D condition
        if selected_pts_mask.sum() + n_init_points <= getattr(self, "max_all_points", 10**9):
            padded_grads_abs[selected_pts_mask] = 0
            mask = (torch.max(self.get_scaling, dim=1).values > self.percent_dense * scene_extent) & (
                padded_max_radii2D > getattr(self, "abs_split_radii2D_threshold", 0)
            )
            padded_grads_abs[~mask] = 0
            selected_pts_mask_abs = torch.where(padded_grads_abs >= grad_abs_threshold, True, False)
            # Limit absolute split points
            max_abs_split_points = getattr(self, "max_abs_split_points", 10**9)
            limited_num = min(getattr(self, "max_all_points", 10**9) - n_init_points - selected_pts_mask.sum(), max_abs_split_points)
            if selected_pts_mask_abs.sum() > limited_num:
                ratio = limited_num / float(max(n_init_points, 1))
            ...
            # Limit by top-k on grad if overflow
            padded_grad[~selected_pts_mask] = 0
            limited_num = getattr(self, "max_all_points", 10**9) - n_init_points
            ratio = limited_num / float(max(n_init_points, 1))
    ...
    def densify_and_prune_pgsr(self, max_grad: float, abs_max_grad: float, min_opacity: float, extent: float, max_screen_size: float):
        grads = self.xyz_gradient_accum / self.denom
        ...
        self.densify_and_clone(grads, max_grad, extent)
        self.densify_and_split_pgsr(grads, max_grad, grads_abs, abs_max_grad, extent, max_radii2D)
