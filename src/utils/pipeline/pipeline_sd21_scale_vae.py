    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.empty_text_embed is None:
            self.encode_empty_text()
        prompt_embeds = self.empty_text_embed.repeat(
            (batch_size, 1, 1)
        ).to(device)  # [B, 2, 1024]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                scaled_latent_model_input = torch.cat([ scaled_latent_model_input, image_latents*0.18215], dim=1)
        ...
        if not output_type == "latent":
            ...
            image, has_nsfw_concept = self.run_safety_checker(image, device, image_latents.dtype)
        else:
            image = latents
