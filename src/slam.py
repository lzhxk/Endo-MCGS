...
from .utils.fov_adaptor import FoVAdaptor
...
class SLAM:
    ...
    def __init__(self, cfg: DictConfig, dataset=None, output_folder: Optional[str] = None):
        ...
        # FoV Adaptation module (triggered after warmup)
        self.fov_adaptor = None
        self.fov_adapted_intrinsics = None
        if cfg.get("use_fov_adaptation", False):
            self.fov_adaptor = FoVAdaptor(
                video=self.video,
                cfg=cfg,
                device=str(self.device),
                rho_min=cfg.get("fov_rho_min", 10.0),
                rho_max=cfg.get("fov_rho_max", 200.0),
                alpha=cfg.get("fov_alpha", 0.3),
                beta=cfg.get("fov_beta", 0.3),
                rho_base=cfg.get("fov_rho_base", 50.0),
                min_keyframes=cfg.get("fov_min_keyframes", 4),
                evaluation_root=os.path.join(self.output, "evaluation") if cfg.get("save_evaluation_intermediate", False) else None,
                flow_vis_max_pairs=cfg.get("fov_flow_vis_max_pairs", None),
                rho_use_deepest_patch=cfg.get("fov_rho_use_deepest_patch", True),
                rho_patch_size=cfg.get("fov_rho_patch_size", 7),
                rho_patch_min_valid_frac=cfg.get("fov_rho_patch_min_valid_frac", 0.5),
            )
            self.info("FoV Adaptation module initialized (will trigger after warmup)")
    ...
    def _maybe_trigger_fov_adaptation(self, stream):
        """Trigger FoV adaptation as early as possible when sufficient data is available.

        FoV adaptation is triggered as soon as counter >= 2, i.e. the first frame pair
        has been processed and flow data is available. This replaces the previous scheme
        that waited for frontend.is_initialized (warmup=8 frames), ensuring corrected
        intrinsics are applied from the beginning of tracking rather than after ~60 frames.
        """
        if self.fov_adaptor is None:
            return

        if self.fov_adaptor.adaptation_done:
            return

        # Require at least 2 frames in video (first frame pair processed)
        if self.video.counter.value < 2:
            return

        # Check if we already tried (failed or succeeded)
        if self.video.fov_adapted[0].item() != 0:
            return

        self.info(
            f"[DEBUG] Intrinsics before FoV adaptation: fx={self.fx:.2f}, fy={self.fy:.2f}, "
            f"cx={self.cx:.2f}, cy={self.cy:.2f}"
        )
        self.info(f"[DEBUG] Video intrinsics[0]: {self.video.intrinsics[0]}")
        self.info(f"[DEBUG] Config intrinsics: fx={self.cfg.data.cam.fx:.2f}, fy={self.cfg.data.cam.fy:.2f}, "
                  f"cx={self.cfg.data.cam.cx:.2f}, cy={self.cfg.data.cam.cy:.2f}")

        self.info("FoV adaptation triggered (early), computing rho...")

        try:
            first_image_path = self._get_first_image_path(stream)
            if first_image_path is None:
                self.info("Could not find first image for FoV adaptation")
                self.video.fov_adapted[0] = 1  # Mark as done (failed)
                return

            intrinsics, K, rho, rho_norm = self.fov_adaptor.adapt_and_update_intrinsics(first_image_path)

            if intrinsics is not None:
                self._update_intrinsics(intrinsics)
                self.fov_adapted_intrinsics = intrinsics
                self.video.fov_adapted[0] = 2  # Mark as successfully adapted

                self.info(
                    f"FoV adaptation complete: rho={rho:.2f}, rho_norm={rho_norm:.4f}, "
                    f"fx={intrinsics['fx']:.2f}, fy={intrinsics['fy']:.2f}, "
                    f"cx={intrinsics['cx']:.2f}, cy={intrinsics['cy']:.2f}"
                )

                # Reset SLAM and reprocess from frame 0 with corrected intrinsics
                self._reset_for_fov_retry(stream)

            else:
                self.info("FoV adaptation failed to compute intrinsics")
                self.video.fov_adapted[0] = 1  # Mark as done (failed)

            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                self.info("CUDA cache cleared after FoV adaptation")

        except Exception as e:
            self.info(f"FoV adaptation error: {e}")
            import traceback
            traceback.print_exc()
            self.video.fov_adapted[0] = 1  # Mark as done (failed)

    def _get_first_image_path(self, stream):
        """Get the path to the first image in the stream for DM-Calib."""
        try:
            # Try to get from stream's input_folder
            if hasattr(stream, 'input_folder') and stream.input_folder:
                import glob
                import os
                input_folder = stream.input_folder
                for ext in ['.jpg', '.jpeg', '.png', '.PNG', '.JPG', '.JPEG']:
                    images = sorted(glob.glob(os.path.join(input_folder, f"*{ext}")))
                    if images:
                        return images[0]
    ...
    def _update_intrinsics(self, intrinsics: dict):
        """Update SLAM system intrinsics with new values from FoV adaptation."""
        # Update self.fx, self.fy, self.cx, self.cy using the same formula as update_cam
        h_edge = self.cfg.data.cam.H_edge
        w_edge = self.cfg.data.cam.W_edge
        H_out = self.cfg.data.cam.H_out
        W_out = self.cfg.data.cam.W_out
        H = float(self.cfg.data.cam.H)
        W = float(self.cfg.data.cam.W)

        fx_new = intrinsics['fx'] * (W_out + w_edge * 2) / W
        fy_new = intrinsics['fy'] * (H_out + h_edge * 2) / H
        cx_new = intrinsics['cx'] * (W_out + w_edge * 2) / W
        cy_new = intrinsics['cy'] * (H_out + h_edge * 2) / H

        self.fx = fx_new - w_edge
        self.fy = fy_new - h_edge
        self.cx = cx_new - w_edge
        self.cy = cy_new - h_edge

        # Update video intrinsics for all existing frames
        # NOTE: video.intrinsics stores intrinsics at 1/scale_factor resolution
        # so we need to divide by scale_factor to match the storage format
        with self.video.get_lock():
            counter = self.video.counter.value
            for i in range(counter):
                self.video.intrinsics[i] = torch.tensor(
                    [self.fx, self.fy, self.cx, self.cy],
                    device=self.video.device
                ) / self.video.scale_factor

        # Update config intrinsics for future use
        self.cfg.data.cam.fx = intrinsics['fx']
        self.cfg.data.cam.fy = intrinsics['fy']
        self.cfg.data.cam.cx = intrinsics['cx']
        self.cfg.data.cam.cy = intrinsics['cy']

        # Update Gaussian Mapper camera intrinsics if mapper exists and has cameras
        if hasattr(self, 'gaussian_mapper') and self.gaussian_mapper is not None:
            mapper_intrinsics = torch.tensor(
                [self.fx, self.fy, self.cx, self.cy],
                device='cuda:0'
            )
            self.gaussian_mapper.update_all_cameras_intrinsics(mapper_intrinsics)

        self.info(
            f"Updated SLAM intrinsics: fx={self.fx:.2f}, fy={self.fy:.2f}, "
            f"cx={self.cx:.2f}, cy={self.cy:.2f}"
        )

    def _reset_for_fov_retry(self, stream):
        """Reset SLAM state to start tracking from frame 0 with corrected intrinsics.

        This is called after FoV adaptation to reprocess the entire video stream
        with the newly calibrated camera intrinsics.
        """
        self.info("FoV adaptation complete: resetting SLAM to reprocess from frame 0 with new intrinsics...")

        # 1. Reset video buffer
        with self.video.get_lock():
            self.video.counter.value = 0
            self.video.ready.value = 0

            # Reset poses to identity
            zero_poses = torch.tensor([0, 0, 0, 0, 0, 0, 1], dtype=torch.float, device=self.device)
            self.video.poses[:] = zero_poses
            self.video.poses_gt[:] = zero_poses
            self.video.pose_changes[:] = zero_poses

            # Reset disparities to default
            self.video.disps[:] = 1.0
            self.video.disps_up[:] = 0.0
            self.video.disps_sens[:] = 0.0
            self.video.disps_sens_up[:] = 0.0

            # Reset intrinsics to new adapted values (at 1/scale_factor resolution)
            new_intr = torch.tensor(
                [self.fx, self.fy, self.cx, self.cy],
                device=self.video.device
            ) / self.video.scale_factor
            self.video.intrinsics[:] = new_intr

            # Reset dirty flags
            self.video.dirty[:] = False
            self.video.mapping_dirty[:] = False

            # Reset flow/translation data
            self.video.flow_norm_sum[:] = 0.0
            self.video.flow_pixel_count[:] = 0.0
            self.video.flow_2d[:] = 0.0
            self.video.translation_norm[:] = 0.0

            # Reset timestamp
            self.video.timestamp[:] = 0.0

        # 2. Reset frontend state
        self.frontend.count = 0
        self.frontend.optimizer.is_initialized = False
        self.frontend.optimizer.count = 0
        self.frontend.optimizer.t0 = 0
        self.frontend.optimizer.t1 = 0
        self.frontend.optimizer.last_pose = None
        self.frontend.optimizer.last_disp = None
        self.frontend.optimizer.last_time = None

        # 3. Reset factor graph - directly reset attributes to avoid None-checking issues in rm_factors
        self.frontend.optimizer.graph.ii = torch.tensor([], dtype=torch.long, device=self.device)
        self.frontend.optimizer.graph.jj = torch.tensor([], dtype=torch.long, device=self.device)
        self.frontend.optimizer.graph.age = torch.tensor([], dtype=torch.long, device=self.device)
        # Reset correlation volume
        self.frontend.optimizer.graph.corr = None
        self.frontend.optimizer.graph.net = None
        self.frontend.optimizer.graph.inp = None
        # Reset inactive factors
        self.frontend.optimizer.graph.ii_inac = torch.tensor([], dtype=torch.long, device=self.device)
        self.frontend.optimizer.graph.jj_inac = torch.tensor([], dtype=torch.long, device=self.device)
        self.frontend.optimizer.graph.ii_bad = torch.tensor([], dtype=torch.long, device=self.device)
        self.frontend.optimizer.graph.jj_bad = torch.tensor([], dtype=torch.long, device=self.device)
        # Reset target/weight
        ht = self.video.ht // 8
        wd = self.video.wd // 8
        self.frontend.optimizer.graph.target = torch.zeros([1, 0, ht, wd, 2], device=self.device, dtype=torch.float)
        self.frontend.optimizer.graph.weight = torch.zeros([1, 0, ht, wd, 2], device=self.device, dtype=torch.float)
        self.frontend.optimizer.graph.target_inac = torch.zeros([1, 0, ht, wd, 2], device=self.device, dtype=torch.float)
        self.frontend.optimizer.graph.weight_inac = torch.zeros([1, 0, ht, wd, 2], device=self.device, dtype=torch.float)
        self.frontend.optimizer.graph.damping = 1e-6 * torch.ones_like(self.video.disps)

        # 4. Reset video retry flag
        self.video.fov_triggered_retry = True

        self.info("SLAM reset complete. Reprocessing video stream from frame 0...")
    ...
    def tracking(
        ...
        # Use while loop with manual iterator so we can restart from frame 0 after FoV adaptation
        stream_iter = iter(stream)
        pbar = tqdm(total=len(stream), desc="Tracking", unit="frame")
        frame_index = 0
        ...
        while True:
            ...
            # FoV Adaptation: trigger as early as possible once we have at least one valid
            # frame pair (counter >= 2). We no longer wait for frontend.is_initialized
            # (warmup=8), since flow data is already available from frame 1 onwards.
            # This ensures corrected intrinsics are used from the very beginning.
            if self.fov_adaptor is not None and not self.fov_adaptor.adaptation_done:
                if self.video.fov_adapted[0].item() == 0 and self.video.counter.value >= 2:
                    self._maybe_trigger_fov_adaptation(stream)

            # Check if FoV adaptation triggered a retry (stream was reset)
            # If so, restart the iterator from the beginning
            if self.video.fov_triggered_retry:
                self.video.fov_triggered_retry = False
                self.info(f"[Main] FoV retry: restarting stream from frame 0 (processed {frame_index - 1} frames before reset)")
                stream_iter = iter(stream)
                frame_index = 0
                pbar.reset()
                continue
    ...
    def evaluate(self, stream, gaussian_mapper_last_state: Optional[eval_utils.EvaluatePacket] = None) -> None:
        ...
        # Log FoV adaptation intrinsics if available (computed in subprocess, read back in main process)
        if self.fov_adaptor is not None and self.fov_adapted_intrinsics is not None:
            ia = self.fov_adapted_intrinsics
            # Read back the actual intrinsics used by video from shared memory
            vid_intr = self.video.intrinsics[0]
            self.info(
                f"[FoV Adaptation] Intrinsics used: "
                f"fx={ia['fx']:.2f}, fy={ia['fy']:.2f}, cx={ia['cx']:.2f}, cy={ia['cy']:.2f} | "
                f"video[0]={vid_intr.tolist()}", logger=log
            )
        elif self.fov_adaptor is not None:
            vid_intr = self.video.intrinsics[0]
            self.info(f"[FoV Adaptation] Not adapted, using defaults: video[0]={vid_intr.tolist()}", logger=log)
        ...
        if self.cfg.run_mapping:
            ...
            kf_rnd_metrics = eval_utils.eval_rendering(
                ...
                use_pgsr_renderer=self.cfg.mapping.get("use_pgsr_renderer", False),
            )
            self.info(
            ...
            # Clear CUDA cache between keyframe and non-keyframe evaluation
            # to prevent memory fragmentation issues with PGSR renderer
            if torch.cuda.is_available():
    ...
    def get_trajectories(self, stream, gaussian_mapper_last_state: Optional[eval_utils.EvaluatePacket] = None):
        ...
        # print("stream.poses", stream.poses[:5])
        if stream.poses is not None:
