...
from .pgsr_renderer import render as pgsr_render
...
def evaluate_evo(
    ...
    try:
        ...
        # Umeyama alignment can fail for degenerate point distributions
        print(colored(f"[Eval] Umeyama alignment failed: {e}. Proceeding without alignment.", "yellow"))
        # Save diagnostic position arrays to help debugging the degenerate input
        try:
            # Save as whitespace-delimited text files for easier inspection
            try:
                # positions_xyz is Nx3 -> save as columns "x y z"
                np.savetxt(
                    os.path.join(plot_dir, "traj_ref_positions.txt"), traj_ref.positions_xyz, fmt="%.6f", header="x y z"
                )
                np.savetxt(
                    os.path.join(plot_dir, "traj_est_positions.txt"), traj_est.positions_xyz, fmt="%.6f", header="x y z"
                )
            except Exception:
                # Fall back to binary format if text saving fails for any reason
                np.save(os.path.join(plot_dir, "traj_ref_positions.npy"), traj_ref.positions_xyz)
                np.save(os.path.join(plot_dir, "traj_est_positions.npy"), traj_est.positions_xyz)
        except Exception:
            pass
...
def eval_ate(
    ...
    assert traj_est.shape[0] >= 3, "Not enough points for alignment"
    ...
    # print("traj_est", traj_est.shape, "traj_gt", traj_gt.shape, "timestamps", len(timestamps))
    ate = evaluate_evo(
        poses_est=traj_est,
        poses_gt=traj_gt,
...
def _to_uint8_image(img: np.ndarray) -> np.ndarray:
    """Convert image to uint8 safely without changing channel order."""
...
def save_dense_predictions(
    ...
    """
    Save predicted RGB and depth without colormap or extra borders.
    Depth is written as float32 PNG (no normalization / colormap / padding).
    """
    # --- RGB ---
    if isinstance(est_img, torch.Tensor):
        est_img_np = est_img.detach().cpu().squeeze().numpy()
    else:
        est_img_np = np.asarray(est_img).squeeze()
    est_img_np = _to_uint8_image(est_img_np)
    # If channels first, transpose to HWC
    if est_img_np.ndim == 3 and est_img_np.shape[0] in (3, 4):
        est_img_np = np.transpose(est_img_np, (1, 2, 0))
    cv2.imwrite(os.path.join(save_dir, f"est_img_{str(idx).zfill(4)}.png"), est_img_np)

    # --- Depth ---
    if isinstance(est_depth, torch.Tensor):
        est_depth_np = est_depth.detach().cpu().squeeze().numpy()
    else:
        est_depth_np = np.asarray(est_depth).squeeze()
    est_depth_np = est_depth_np.astype(np.float32)
    est_depth_np = np.nan_to_num(est_depth_np, nan=0.0, posinf=0.0, neginf=0.0)
    # Save raw depth values directly, no normalization or colormap
    # Save raw depth as float32 tiff (no normalization/colormap)
    cv2.imwrite(os.path.join(save_dir, f"est_depth_{str(idx).zfill(4)}.tiff"), est_depth_np)
...
def do_odometry_evaluation(
    ...
    # print("gt_c2w_kf_lie", gt_c2w_kf_lie)
...
def eval_rendering(
    ...
    for i, idx in tqdm(enumerate(tstamps)):
        ...
        try:
            ...
            if use_pgsr_renderer:
                render_dict = pgsr_render(cam, gaussians, render_pipeline_cfg, background, return_plane=True, return_depth_normal=False)
            else:
            ...
            if save_predictions:
                if has_gt_depth:
                    ...
                    print("depth_est_visu.shape", depth_est_visu.shape, depth_est_visu.dtype, depth_est_visu.min(), depth_est_visu.max())
            ...
            # This is especially important for PGSR renderer which allocates/deallocates frequently
            if (i + 1) % 20 == 0:
                torch.cuda.synchronize()
                torch.cuda.empty_cache()

            # Explicitly delete large tensors to free memory
            del image_est, depth_est, render_dict
            del gt_image, gt_depth, gt_img_np, est_img_np
            if has_gt_depth:
                del depth_est_visu

        except Exception as e:
            print(colored(f"[Eval] Warning: Failed to process frame {idx} (tstamps[{i}]): {e}", "yellow"))
