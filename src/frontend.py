...
class FrontendWrapper(torch.nn.Module):
    ...
    def is_initialized(self):
        return self.optimizer.is_initialized

    @property
    def warmup(self):
        return self.optimizer.warmup
...
class Frontend:
    ...
    def __update(self, pose_interpolation: str = "linear", lock: Optional[mp.Lock] = None):
        ...
        with lock:
            ...
            t_norm = dP.translation().norm().item()
            self.video.translation_norm[self.t1 - 1] = t_norm
    ...
    def __initialize(self):
        ...
        # FoV Adaptation: store translation norm for the initialized frames
        # Note: we exclude the last frame (t1) because its pose is a copy of t1-1, giving zero translation
        with self.video.get_lock():
            for i in range(self.t1 - 1):
                # Translation of frame i relative to frame i-1
                dP_init = SE3(self.video.poses[i + 1]) * SE3(self.video.poses[i]).inv()
                t_norm = dP_init.translation().norm().item()
                self.video.translation_norm[i + 1] = t_norm
            # Frame 0 has no previous frame, set to 0
            self.video.translation_norm[0] = 0.0
