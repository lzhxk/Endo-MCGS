...
from .losses.pgsr_losses import pgsr_mapping_loss, multi_view_consistency_loss
...
class GaussianMapper(object):
    ...
    def __init__(self, cfg, slam, gui_qs=None):
        ...
        # PGSR integration options
        self.use_pgsr_renderer = cfg.mapping.get("use_pgsr_renderer", False)
        self.use_pgsr_loss = cfg.mapping.get("use_pgsr_loss", False)
        
        # Check if PGSR is actually available
        try:
            from .gaussian_splatting.pgsr_renderer import PGSR_AVAILABLE
            if self.use_pgsr_renderer and not PGSR_AVAILABLE:
                self.info("PGSR renderer requested but not available, falling back to original renderer")
                self.use_pgsr_renderer = False
                self.use_pgsr_loss = False
        except ImportError:
            if self.use_pgsr_renderer:
                self.info("PGSR renderer requested but not available, falling back to original renderer")
                self.use_pgsr_renderer = False
                self.use_pgsr_loss = False

        # Manifold projection (SDF-based Gaussian projection)
        from .utils.manifold_projection import ManifoldProjectionController
        self.manifold_ctrl = ManifoldProjectionController(cfg)
        if self.manifold_ctrl.enabled:
            self.info(f"ManifoldProjection enabled (backward_mode={self.manifold_ctrl.backward_mode})")

        # Tangent space gradient projection (Riemannian optimization)
        self.tangent_projection_enabled = cfg.mapping.get("tangent_projection", {}).get("enabled", False)
        if self.tangent_projection_enabled:
            self.info("Tangent space gradient projection enabled")
        ...
        if self.manifold_ctrl.enabled:
            self.gaussians.set_manifold_controller(self.manifold_ctrl)
        ...
        self.manifold_ctrl.training_setup(self.opt_params)
        ...
        self.count = 0
        self._plane_prune_cursor = 0  # Round-robin index for plane pruning frames

    def update_all_cameras_intrinsics(self, intrinsics: torch.Tensor):
        """Update intrinsics for all existing cameras in the mapper.

        This is called when FoV adaptation updates the intrinsics to ensure
        all existing cameras use the new intrinsics.

        The intrinsics stored in each camera follow the same logic as when
        cameras are created in add_new_gaussians:
        - If video.upsampled: camera stores intrinsics at output resolution (video.intrinsics[0])
        - Otherwise: camera stores intrinsics at downsample resolution (video.intrinsics[0])

        Args:
            intrinsics: [fx, fy, cx, cy] tensor at output resolution
        """
        # intrinsics is already at output resolution (stored in self.fx, self.fy, self.cx, self.cy)
        # This is the same as video.intrinsics[0]
        fx, fy, cx, cy = intrinsics

        updated_count = 0
        for cam in self.cameras:
            cam.update_intrinsics(
                (fx, fy, cx, cy),
        ...
        self.info(f"Updated intrinsics for {updated_count} cameras")

    def info(self, msg: str):
        print(colored("[Gaussian Mapper] " + msg, "magenta"))

    def _get_nearest_camera(self, view: Camera) -> Optional[Camera]:
        """Pick a nearest camera to the given view for multi-view consistency.
        Strategy: choose camera with closest uid different from current.
        """
        ...
        for cam in self.cameras:
            if cam.uid == view.uid:
                continue
            dist = abs(int(cam.uid) - int(view.uid))
            if dist < best_dist:
                best = cam
                best_dist = dist
        return best
    ...
    def save_render(self, cam: Camera, render_path: str) -> None:
        ...
        # Use PGSR renderer if enabled
        if self.use_pgsr_renderer:
            render_pkg = pgsr_render(cam, self.gaussians, self.pipeline_params, self.background, return_plane=True, return_depth_normal=False)
        else:
    ...
    def get_mapping_update(
        ...
        just does not converge correctly. The Tracker operates with dense depth frames, so we need to be careful to not feedback too sparse depths.
        ...
        for view in frames:
            ...
            if self.cam2buffer[view.uid] in ignore_frames:
                ...
                continue
            if self.use_pgsr_renderer:
                render_pkg = pgsr_render(view, self.gaussians, self.pipeline_params, self.background, return_plane=True, return_depth_normal=False)
            else:
            ...
            depth = render_pkg.get("depth", render_pkg.get("plane_depth")).detach()
            # Filter away pixels with very low opacity as these are usually unreliable
            valid_o = render_pkg.get("opacity", torch.ones_like(depth)).detach() > opacity_threshold
            depth[~valid_o] = 0.0
    ...
    def render_compare(self, view: Camera) -> Tuple[float, Dict, Dict]:
        ...
        if self.use_pgsr_renderer:
            render_pkg = pgsr_render(view, self.gaussians, self.pipeline_params, self.background, 
        ...
        if self.use_pgsr_loss and self.use_pgsr_renderer:
            # Use PGSR loss with additional geometric information
            current_loss = pgsr_mapping_loss(
                render_pkg["render"], 
                render_pkg.get("plane_depth", render_pkg.get("depth")), 
                view,
            ...
            # Use original EDroidPGSR loss
            image, depth = render_pkg["render"], render_pkg.get("depth", render_pkg.get("plane_depth"))
    ...
    def mapping_step(
        ...
        viewspace_point_tensor_abs_acm = []
        for view in frames:
            ...
            # Add multi-view geometric & photometric consistency (PGSR)
            # print("view.uid", view.uid)
            # if self.use_pgsr_loss and self.use_pgsr_renderer and view.uid == 0 and \
            #    (self.count > self.loss_params.get('multi_view_weight_from_iter', 4000)):
            #     nearest_cam = self._get_nearest_camera(view)
            #     if nearest_cam is not None:
            #         print(f"multi_view_consistency_loss;")
            #         mv_loss = multi_view_consistency_loss(
            #             viewpoint_cam=view,
            ...
            #             iteration=self.count,
            #         )
            #         current_loss = current_loss + mv_loss
            ...
            if "viewspace_points_abs" in render_pkg:
                viewspace_point_tensor_abs_acm.append(render_pkg["viewspace_points_abs"])
            else:
                viewspace_point_tensor_abs_acm.append(None)
        ...
        # SDF manifold eikonal regularizer
        if self.manifold_ctrl.enabled:
            sdf_reg = self.manifold_ctrl.sdf_regularization(self.gaussians.get_xyz)
            loss += sdf_reg
        ...
        # This projects the Euclidean gradient onto the manifold tangent space
        # ensuring that Gaussian centers always remain on the surface.
        # grad_tangent = (I - nn^T) @ grad_xyz
        if self.tangent_projection_enabled and self.manifold_ctrl.enabled:
            xyz_grad = self.gaussians._xyz.grad
            if xyz_grad is not None and xyz_grad.numel() > 0:
                normals = self.manifold_ctrl.get_normals(self.gaussians._xyz, normalize=True)
                dot_products = (xyz_grad * normals).sum(dim=-1, keepdim=True)
                self.gaussians._xyz.grad = xyz_grad - dot_products * normals
        ...
        with torch.no_grad():
            for idx in range(len(viewspace_point_tensor_acm)):
                ...
                if self.update_params.densify.accumulate_pixels:
                    ...
                    pixels = None
                # DROID-Splat style: optionally weight by number of touched pixels
                if self.use_pgsr_renderer and viewspace_point_tensor_abs_acm[idx] is not None:
                    self.gaussians.add_densification_stats_pgsr(
                        viewspace_point_tensor_acm[idx], viewspace_point_tensor_abs_acm[idx], visibility_filter_acm[idx]
                    )
                else:
            ...
            if self.last_idx > self.n_last_frames and prune_densify:
                ...
                van = self.update_params.densify.vanilla
                scale_std = getattr(van, "scale_std", 1.0)
                if self.use_pgsr_renderer:
                    abs_max_grad = getattr(van, "abs_max_grad", van.max_grad)
                    self.gaussians.densify_and_prune_pgsr(van.max_grad, abs_max_grad, van.min_opacity, van.extent, van.max_screen_size)
                else:
                    self.gaussians.densify_and_prune(van.max_grad, van.min_opacity, van.extent, van.max_screen_size, scale_std)

        ### Update states
        self.gaussians.optimizer.step()
        self.gaussians.optimizer.zero_grad()
        self.manifold_ctrl.step()
    ...
    def covisibility_pruning(
        ...
        for view in frames:
            # Use PGSR renderer if enabled for visibility estimation as well
            if self.use_pgsr_renderer:
                render_pkg = pgsr_render(view, self.gaussians, self.pipeline_params, self.background, return_plane=False, return_depth_normal=False)
            else:
    ...
    def plane_consistency_pruning(
        self,
        frames: List[Camera],
        depth_threshold_ratio: float = 0.5,
        opacity_reduction: float = 0.5,
        max_frames: int = 3,
    ):
        """
        实现“平面一致性剪枝”（Plane-Consistency Pruning）。

        对于给定视图中的每个像素：
          1）根据沿该像素射线可见的 Gaussians 的深度，只取最近（深度最小）的一个作为“第一平面”；
          2）在以该像素为中心的 3×3 邻域内，统计周围 8 个像素各自“第一平面”深度与中心像素“第一平面”深度的差值；
          3）若这 8 个深度差值的平均值 > depth_threshold_ratio * 全图深度范围，
             则认为该中心像素的“第一平面”Gaussian 不与邻域平面一致，对其进行剪枝（通过降低透明度实现）。

        Args:
            frames: 要处理的相机视图列表
            depth_threshold_ratio: 深度差值阈值比例（相对于全图有效深度范围）
            opacity_reduction: 剪枝时透明度缩放因子（0-1 之间，越小剪枝越强）
        """
        if len(frames) == 0:
            return
        ...
        if len(frames) > max_frames:
            cursor = getattr(self, "_plane_prune_cursor", 0) % len(frames)
            frames_to_process = []
            for _ in range(max_frames):
                frames_to_process.append(frames[cursor])
                cursor = (cursor + 1) % len(frames)
            self._plane_prune_cursor = cursor

        self.info(
            f"[PlanePrune] Start plane consistency pruning over {len(frames_to_process)} frame(s) "
            f"(depth_ratio={depth_threshold_ratio}, opacity_scale={opacity_reduction})"
        )

        gaussian_opacity_reductions = torch.ones(len(self.gaussians), device=self.device)
        total_pruned_pixels = 0

        for view in frames_to_process:
            view_uid = getattr(view, "uid", None)
            view_tag = f"uid={view_uid}" if view_uid is not None else "unknown_uid"
            # 渲染当前视图，获取深度与可见性信息
            if self.use_pgsr_renderer:
                render_pkg = pgsr_render(
            ...
            if render_pkg is None:
                self.info(f"[PlanePrune] Skip view {view_tag}: render_pkg is None")
                continue
                
            depth_map = render_pkg.get("plane_depth", render_pkg.get("depth"))
            if depth_map is None:
                self.info(f"[PlanePrune] Skip view {view_tag}: no depth map")
                continue
            ...
            # 获取可见的 Gaussians（布尔 mask，长度等于全局 Gaussians 数量）
            visibility_filter = render_pkg.get(
                "visibility_filter",
                torch.ones(len(self.gaussians), dtype=torch.bool, device=self.device),
            )
            visible_gaussians = torch.where(visibility_filter)[0]
            
            if len(visible_gaussians) == 0:
                self.info(f"[PlanePrune] Skip view {view_tag}: no visible Gaussians")
                continue
            
            # 计算每个可见 Gaussian 在相机坐标系下的深度
            gaussian_xyz = self.gaussians.get_xyz[visible_gaussians]  # (N, 3)
            
            # 将 Gaussians 转换到相机坐标系
            w2c = view.world_view_transform
            gaussian_xyz_homo = torch.cat([
                gaussian_xyz, 
                torch.ones(gaussian_xyz.shape[0], 1, device=gaussian_xyz.device)
            ], dim=1)  # (N, 4)
            gaussian_xyz_cam = (w2c @ gaussian_xyz_homo.T).T[:, :3]  # (N, 3)
            gaussian_depths = gaussian_xyz_cam[:, 2]  # (N,) 相机坐标系下的 Z 深度
            
            # 将 Gaussians 投影到图像平面
            fx, fy, cx, cy = view.fx, view.fy, view.cx, view.cy
            gaussian_uv = torch.zeros(len(visible_gaussians), 2, device=self.device)
            valid_depth_mask = gaussian_depths > 0
            gaussian_uv[valid_depth_mask, 0] = fx * gaussian_xyz_cam[valid_depth_mask, 0] / gaussian_depths[valid_depth_mask] + cx
            gaussian_uv[valid_depth_mask, 1] = fy * gaussian_xyz_cam[valid_depth_mask, 1] / gaussian_depths[valid_depth_mask] + cy
            
            # 获取每个 Gaussian 的半径（在图像平面上的投影大小）
            radii = render_pkg.get("radii", torch.zeros(len(self.gaussians), device=self.device))
            gaussian_radii = radii[visible_gaussians]  # (N,)
            
            # 计算当前视图中的总体有效深度范围
            valid_depth = depth_map[depth_map > 0]
            if len(valid_depth) == 0:
                self.info(f"[PlanePrune] Skip view {view_tag}: no valid depth pixels")
                continue
            depth_range = valid_depth.max() - valid_depth.min()
            if depth_range <= 0:
                self.info(f"[PlanePrune] Skip view {view_tag}: zero depth range")
                continue
            depth_threshold = depth_range * depth_threshold_ratio
            
            first_plane_gaussian_map = torch.full((H, W), -1, dtype=torch.long, device=device)
            first_plane_depth_map = torch.full((H, W), float("inf"), dtype=depth_map.dtype, device=device)
            x_coords = torch.arange(W, device=device, dtype=depth_map.dtype).view(1, W)
            y_coords = torch.arange(H, device=device, dtype=depth_map.dtype).view(H, 1)

            for idx, gaussian_idx in enumerate(visible_gaussians):
                if not valid_depth_mask[idx]:
                    continue

                u = gaussian_uv[idx, 0]
                v = gaussian_uv[idx, 1]
                depth = gaussian_depths[idx]
                radius = max(gaussian_radii[idx].item(), 2.0)

                u_min = max(0, int(torch.floor(u - radius).item()))
                u_max = min(W, int(torch.ceil(u + radius).item()) + 1)
                v_min = max(0, int(torch.floor(v - radius).item()))
                v_max = min(H, int(torch.ceil(v + radius).item()) + 1)
                if u_min >= u_max or v_min >= v_max:
                    continue

                yy = y_coords[v_min:v_max] - v
                xx = x_coords[:, u_min:u_max] - u
                dist2 = yy.pow(2) + xx.pow(2)
                influence_mask = dist2 <= (radius * radius)
                if not influence_mask.any():
                    continue

                patch_depth = first_plane_depth_map[v_min:v_max, u_min:u_max]
                patch_gaussian = first_plane_gaussian_map[v_min:v_max, u_min:u_max]
                update_mask = influence_mask & (depth < patch_depth)
                if not update_mask.any():
                    continue

                first_plane_depth_map[v_min:v_max, u_min:u_max] = torch.where(
                    update_mask, depth, patch_depth
                )
                first_plane_gaussian_map[v_min:v_max, u_min:u_max] = torch.where(
                    update_mask, gaussian_idx, patch_gaussian
                )

            valid_mask = torch.isfinite(first_plane_depth_map) & (first_plane_depth_map > 0)
            if valid_mask.sum() == 0:
                self.info(f"[PlanePrune] View {view_tag}: no valid first-plane pixels")
                continue

            depth_pad = torch.full(
                (H + 2, W + 2), float("inf"), dtype=first_plane_depth_map.dtype, device=device
            )
            depth_pad[1:-1, 1:-1] = first_plane_depth_map
            mask_pad = torch.zeros((H + 2, W + 2), dtype=torch.bool, device=device)
            mask_pad[1:-1, 1:-1] = valid_mask
            interior_mask = torch.zeros_like(valid_mask)
            if H > 2 and W > 2:
                interior_mask[1:-1, 1:-1] = True

            diff_sum = torch.zeros_like(first_plane_depth_map)
            neighbor_count = torch.zeros_like(first_plane_depth_map, dtype=torch.int32)

            for dv in (-1, 0, 1):
                for du in (-1, 0, 1):
                    if dv == 0 and du == 0:
                        continue
                    neighbor_depth = depth_pad[1 + dv : 1 + dv + H, 1 + du : 1 + du + W]
                    neighbor_valid = mask_pad[1 + dv : 1 + dv + H, 1 + du : 1 + du + W]
                    valid_pair = valid_mask & neighbor_valid & interior_mask
                    if not valid_pair.any():
                        continue
                    diff = torch.abs(neighbor_depth - first_plane_depth_map)
                    diff_sum += diff * valid_pair.float()
                    neighbor_count += valid_pair.int()

            enough_neighbors = neighbor_count >= 4
            if not enough_neighbors.any():
                self.info(f"[PlanePrune] View {view_tag}: insufficient neighbor support")
                continue

            avg_depth_diff = torch.zeros_like(first_plane_depth_map)
            avg_depth_diff[enough_neighbors] = diff_sum[enough_neighbors] / neighbor_count[
                enough_neighbors
            ].float()
            flag_mask = (avg_depth_diff > depth_threshold) & enough_neighbors & interior_mask
            flagged_pixels = flag_mask.sum().item()
            if flagged_pixels == 0:
                self.info(
                    f"[PlanePrune] View {view_tag}: "
                    f"visible={len(visible_gaussians)}, plane_pruned=0, pixels_flagged=0"
                )
                continue

            total_pruned_pixels += flagged_pixels
            flagged_gaussians = first_plane_gaussian_map[flag_mask]
            flagged_gaussians = flagged_gaussians[flagged_gaussians >= 0]
            if flagged_gaussians.numel() == 0:
                self.info(
                    f"[PlanePrune] View {view_tag}: "
                    f"visible={len(visible_gaussians)}, plane_pruned=0, pixels_flagged={flagged_pixels}"
                )
                continue

            pruned_gaussians_this_view = torch.unique(flagged_gaussians).tolist()
            for gaussian_idx in pruned_gaussians_this_view:
                gaussian_opacity_reductions[gaussian_idx] *= opacity_reduction

            self.info(
                f"[PlanePrune] View {view_tag}: "
                f"visible={len(visible_gaussians)}, "
                f"plane_pruned={len(pruned_gaussians_this_view)}, "
                f"pixels_flagged={flagged_pixels}"
            )
        
        # 应用透明度降低（剪枝）
        if total_pruned_pixels > 0:
            with torch.no_grad():
                current_opacity = self.gaussians.get_opacity
                new_opacity = current_opacity * gaussian_opacity_reductions.unsqueeze(1)
                # 使用 inverse_sigmoid 来更新内部 opacity 参数
                from .gaussian_splatting.utils.general_utils import inverse_sigmoid
                self.gaussians._opacity = torch.nn.Parameter(inverse_sigmoid(new_opacity))
        
        end = time.time()
        self.info(
            f"Plane consistency pruning took {(end - start):.2f}s, "
            f"pruned {total_pruned_pixels} pixels across {len(frames_to_process)} view(s)"
        )
        
        # 可选：完全移除透明度很低的 Gaussians
        if total_pruned_pixels > 0:
            min_opacity = 0.01  # 最小透明度阈值
            very_low_opacity = (self.gaussians.get_opacity < min_opacity).squeeze()
            if very_low_opacity.sum() > 0:
                self.gaussians.prune_points(very_low_opacity)
                self.info(f"Additionally removed {very_low_opacity.sum()} Gaussians with opacity < {min_opacity}")
    ...
    def _last_call(self, mapping_queue: mp.Queue, received_item: mp.Event):
        ...
        # Additionally export cameras for later TSDF fusion based mesh extraction (PGSR-style),
        # without changing the original final ply saving behavior.
        try:
            os.makedirs(f"{self.output}/mesh", exist_ok=True)
            # Prefer only keyframes for stable fusion
            only_kf = [cam for cam in self.cameras if cam.uid in self.cam2buffer]
            cameras_to_save = only_kf if len(only_kf) > 0 else self.cameras
            cam_pack = []
            ...
            np.savez_compressed(f"{self.output}/mesh/cameras_for_tsdf.npz", cameras=cam_pack)
            self.info(f"Exported {len(cam_pack)} cameras for TSDF fusion: {self.output}/mesh/cameras_for_tsdf.npz")
        except Exception as e:
            self.info(f"Failed to export cameras for TSDF fusion: {e}")
    ...
    def _update(self, delay_to_tracking=True, iters: int = 10, release_cache: bool = False):
        ...
        # Disabled to avoid affecting tracking input during PGSR mapping
        ...
        if len(self.iteration_info) % self.update_params.prune_every == 0 and delay_to_tracking:
            ...
            # 可选：在同一频率下执行平面一致性剪枝（如果在配置中开启）
            use_plane_consistency = getattr(self.update_params.pruning, "use_plane_consistency", False)
            if use_plane_consistency:
                # 仅使用当前被选为 keyframe 的视图进行平面一致性检查
                keyframes_for_plane, _ = self.select_keyframes()
                if len(keyframes_for_plane) > 0:
                    self.plane_consistency_pruning(
                        keyframes_for_plane,
                        depth_threshold_ratio=0.03,
                        opacity_reduction=0.1,
                    )
